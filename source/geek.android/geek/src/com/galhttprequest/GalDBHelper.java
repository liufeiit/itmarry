...
import com.guide.geek.utils.ILog;
...
public class GalDBHelper extends SQLiteOpenHelper {
	...
	public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
		ILog.i(Tag, "数据库更新到 version" + newVersion);
		// TODO 更新时做点什么
	}
	...
	public  synchronized int deleteURL(String url) {
		...
		ILog.i("affect +" + res + " row data， delete url =" + url
				+ " successfully!");
		return res;
	}
	...
	public  synchronized int  clear() {
		...
		ILog.i("affect +" + res + " row data， delete table =" + tb_name
				+ " successfully!");
		return res;
	}
	...
	public synchronized void close() {
		try {
			...
			ILog.e(new Throwable().getStackTrace()[0].toString()
					+ " Exception ", e);
		}
	}
	...
	public  synchronized  boolean insertURL(GALURL galurl) {
		try {
			...
			if (err == -1) {
				ILog.i("Error from insertURL:" + err);
				return false;
			} else {
				ILog.i("insertURL successful! ");
				return true;
			}
		} catch (Exception e) {
			ILog.e("Error from insertURL:" + e.toString());
			return false;
		}
	}
