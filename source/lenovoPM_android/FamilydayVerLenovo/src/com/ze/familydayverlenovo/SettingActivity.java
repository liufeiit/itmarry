...
import com.ze.commontool.EditTextTools;
...
import com.ze.familydayverlenovo.adapter.PhoneListViewAdapter;
...
public class SettingActivity extends Activity {
		...
		private 			View 					setting_body_base_1_name;
		private 			View 					setting_body_base_1_head;
		@Override
		protected void onCreate(Bundle savedInstanceState) {
			...
			setting_body_base_1_head = findViewById(R.id.setting_body_base_1_head);
			setting_body_base_1_name = findViewById(R.id.setting_body_base_1_name);
			int len = 0;
			...
			LayoutParams params = new LayoutParams( (len-40)/3, ( len - 40 )/3 );
			setting_body_base_1_head.setLayoutParams(params);
			params = new LayoutParams( (int) ((len-40)*(2.0f/3.0f)), ( len - 40 )/3 );
			params.setMargins(20, 10, 20, 10);
			setting_body_base_1_name.setLayoutParams(params);
			initInfoFromCache();
			loadTask();
		}
		public void initInfoFromCache()
		{
			Componet tmpComponet ;
			tmpComponet = UserInfoManager.getInstance(SettingActivity.this).getItem("name");
			if( tmpComponet != null )
			{
				mName.setText( tmpComponet.getValue() );
			}
			tmpComponet = UserInfoManager.getInstance(SettingActivity.this).getItem("username");
			if( tmpComponet != null )
			{
				mPhone.setText( tmpComponet.getValue() );
			}
			tmpComponet = UserInfoManager.getInstance(SettingActivity.this).getItem("birth");
			if( tmpComponet != null )
			{
				mBirth.setText( tmpComponet.getValue() );
			}
			tmpComponet = UserInfoManager.getInstance(SettingActivity.this).getItem("coin");
			if( tmpComponet != null )
			{
				mCoin.setText( tmpComponet.getValue() );
			}
		}
		@Override
				...
				protected void onPostExecute(String result) {
					...
					try {
						...
						if ( 0 == jsonObject.getInt("error") )
						{
							...
							nameComponet.setValue( jsonObject.getString("name") );
							UserInfoManager.getInstance(SettingActivity.this).add(nameComponet);
							
							Componet birthComponet = new Componet("birth");
							birthComponet.setValue( jsonObject.getString("birthday") );
							UserInfoManager.getInstance(SettingActivity.this).add(birthComponet);
							
							Componet coinComponet = new Componet("coin");
							coinComponet.setValue( jsonObject.getString("credit") );
							UserInfoManager.getInstance(SettingActivity.this).add(coinComponet);
							
							UserInfoManager.getInstance(SettingActivity.this).saveAll();
...
//		private EditText			setBirEditText;
		//private AlertDialog.Builder updateNameDialog;
		private OnClickListener buttonClickListener = new OnClickListener(){
			...
			public void onClick(View v) {
				...
				if ( v == mName ) {
					...
					setNameEditText.setSelection(mName.length());
					setNameEditText.setHint(getResources().getString(R.string.dialog_hint_setname));
					new AlertDialog.Builder(SettingActivity.this).setTitle("修改用户名！").setView(setNameEditText)
					...
					if (setNameAlertDialog == null ) {
						...
						setNameAlertDialog.setOnShowListener(new OnShowListener() {
							
							@Override
							public void onShow(DialogInterface dialog) {
								// TODO Auto-generated method stub
								EditTextTools.showInputMethod(SettingActivity.this, setNameEditText, -1);
							}
						});
						/*setNameAlertDialog.setOnDismissListener(new OnDismissListener() {
							
							@Override
							public void onDismiss(DialogInterface dialog) {
								// TODO Auto-generated method stub
								//EditTextTools.hideInputMethod(SettingActivity.this, setNameEditText);
							}
						});*/
					}
					setNameEditText.setText(mName.getText());
					...
					if ( setBirthAlertDialog == null ) {
						OnDateSetListener onDateSetListener = new OnDateSetListener() {
							...
							public void onDateSet(DatePicker view, int year, int monthOfYear,
								...
								set_date.setYear(year-1900);
								set_date.setMonth(monthOfYear);
								set_date.setDate(dayOfMonth);
								set_date.setHours(0);
								set_date.setMinutes(0);
								set_date.setSeconds(0);
								if ( set_date.before(new Date())) {
									 setBirthTask(year, monthOfYear + 1, dayOfMonth);
								}else
								{
									ToastUtil.show(SettingActivity.this, "选中的日期不能超过今天的日期!");
								}
...
//					Toast.makeText(SettingActivity.this, R.string.notopen, Toast.LENGTH_SHORT).show();
					...
					intent.setClass(SettingActivity.this, PhotoShowActivity.class)	;
					startActivity(intent);
				}else if( v == mAbout )
				{
//					Toast.makeText(SettingActivity.this, R.string.notopen, Toast.LENGTH_SHORT).show();
...
//					 Toast.makeText(this, text, duration)
				 ...
				 {
					 // 拍照后返回的Intent不管有没有拍照（按下了拍照或者按下了返回），都为null，此时如果有拍照，resultCode不等于0
					 // 如果没拍照,resultCode=0
					 return;
				 }
			 }
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				if( dialog == setNameAlertDialog )
				{
					if (which == DialogInterface.BUTTON_POSITIVE) {
				        ...
				        imm.hideSoftInputFromWindow(setNameEditText.getWindowToken(),0);
						setNameTask();
