...
public class MainView extends View {
	...
	private final Bitmap 
			mouse = decodeResource(getResources(), R.drawable.mouse),
			cow = decodeResource(getResources(), R.drawable.cow),
			tiger = decodeResource(getResources(), R.drawable.tiger),
			rabbit = decodeResource(getResources(), R.drawable.rabbit),
			dragon = decodeResource(getResources(), R.drawable.dragon),
			snake = decodeResource(getResources(), R.drawable.snake),
			horse = decodeResource(getResources(), R.drawable.horse),
			sheep = decodeResource(getResources(), R.drawable.sheep),
			monkey = decodeResource(getResources(), R.drawable.monkey),
			chicken = decodeResource(getResources(), R.drawable.chicken),
			dog = decodeResource(getResources(), R.drawable.dog),
			pig = decodeResource(getResources(), R.drawable.pig)
			;
	
//	private Bitmap d(Resources resources, int id) {
//		Bitmap bmp = BitmapFactory.decodeResource(resources, id);
//		Bitmap newb = Bitmap.createBitmap(cellSize, cellSize, Config.ARGB_8888);
...
//	}
	...
	private Bitmap decodeResource(Resources resources, int id) {
	    ...
	    BitmapFactory.Options opts = setBitmapOption(resources, id, cellSize, cellSize);
	    opts.inTargetDensity = value.density;
	    return BitmapFactory.decodeResource(resources, id);
	}
	
	private static BitmapFactory.Options setBitmapOption(Resources resources, int id, int width, int height) {
...
//  GameView.drawImage(canvas, mBitDestTop, miDTX, mBitQQ.getHeight(), mBitDestTop.getWidth(), mBitDestTop.getHeight()/2, 0, 0);
  public void drawImage(Canvas canvas, Bitmap blt, int x, int y, int w, int h, int bx, int by) {//x,y表示绘画的起点，
      ...
      canvas.drawBitmap(blt, src, dst, paint);//这个方法  第一个参数是图片原来的大小，第二个参数是 绘画该图片需显示多少。也就是说你想绘画该图片的某一些地方，而不是全部图片，第三个参数表示该图片绘画的位置
      src = null;
      dst = null;
  }
...
//			drawImage(canvas, getBitmap(value), 0, 0, cellSize, cellSize, 0, 0);
			RectF rectF = new RectF(0, 0, cellSize, cellSize);
			canvas.drawBitmap(getBitmap(value), null, rectF, null);
